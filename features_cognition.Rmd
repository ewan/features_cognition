...
example_pair_info <- melt(mutate(test_pairs, pair=paste0("Pair", 1:nrow(test_pairs))),
                          measure.vars = c("x1","x2"),
                          value.name = "label",
                          variable.name = "Phone") %>%
                     mutate(Feature=factor(map_chr(as.character(fname), ~ if (. == "Nasal") . else "Other"),
...
We use as reference pairs all the other pairs that appear somewhere in Table XXX. The mutual similarity of the vector subtractions corresponding to [nasal], as a group, as regards their direction, is expected to be statistically greater than the similarity of [nasal] subtractions with subtractions derived from reference pairs. A single pair of subtractions can be compared as regards their direction by taking the cosine of the angle between them. Values close to zero indicate nearly orthogonal vectors; values close to one indicate vectors in nearly the same direction; values close to negative one indicate vectors in nearly opposite directions. Figure XXX shows (in dark blue) the nasal subtraction vectors from Figure XXX, placed at the origin and normalized to length one to illustrate the differences and similarities in their direction; in light orange are shown the reference pairs. The cosine of the angle between [d]/[n] and [ɡ]/[ŋ] here is
`r cossim(with(example_pairs[example_pairs$label.x1=="ɡ",], c(PC1.x2-PC1.x1, PC2.x2-PC2.x1)), with(example_pairs[example_pairs$label.x2=="n",], c(PC1.x2-PC1.x1, PC2.x2-PC2.x1))) %>% round(digits=3)`,
while the cosine of the angle between [d]/[n] and [p]/[k] is
`r cossim(with(example_pairs[example_pairs$label.x1=="p" & example_pairs$label.x2=="k",], c(PC1.x2-PC1.x1, PC2.x2-PC2.x1)), with(example_pairs[example_pairs$label.x2=="n",], c(PC1.x2-PC1.x1, PC2.x2-PC2.x1))) %>% round(digits=3)`.

```{r mptest-example-pairs, echo=F, fig.width=5, fig.height=5}
example_pairs$xend <- with(example_pairs, (PC1.x2-PC1.x1)/norm)
example_pairs$yend <- with(example_pairs, (PC2.x2-PC2.x1)/norm)
example_pairs$text_x <- with(example_pairs, xend*1.1)
example_pairs$text_y <- with(example_pairs, yend*1.1)
example_pairs$text_x[example_pairs$label.x1=="ɡ"] <-
        example_pairs$text_x[example_pairs$label.x1=="ɡ"] - 0.04
example_pairs$text_y[example_pairs$label.x1=="ɡ"] <-
        example_pairs$text_y[example_pairs$label.x1=="ɡ"] + 0.11
fig_mptest_pairs <- ggplot(example_pairs, aes(x=PC1, y=PC2, label=label)) +
  geom_segment(data=filter(example_pairs, Feature=="Other"),
               aes(x=0, xend=xend, y=0, yend=yend, label=NA, colour=Feature),
               lwd=1.7,
               arrow=arrow(type="closed", length=unit(0.2, "inches"))) +  
  geom_segment(data=filter(example_pairs, Feature=="Nasal"),
               aes(x=0, xend=xend, y=0, yend=yend, label=NA, colour=Feature),
               lwd=1.7,
               arrow=arrow(type="closed", length=unit(0.2, "inches"))) +  
  geom_segment(data=filter(example_pairs, Feature=="Nasal"),
               aes(x=0, xend=xend, y=0, yend=yend, label=NA),
               lwd=0.5) +  
  geom_point(aes(x=0,y=0,label=NA), size=2.5) +
  geom_point(data=filter(example_pairs, Feature=="Nasal"),
             aes(x=xend,y=yend,label=NA), size=2.5) +
  geom_text(data=filter(example_pairs, Feature=="Nasal"), aes(x=text_x,y=text_y,
                label=pair_labels(label.x1, label.x2, "/")), size=6) +
  geom_segment(data=filter(example_pairs, label.x1=="p", label.x2=="k"),
               aes(x=0, xend=xend, y=0, yend=yend, label=NA),
               lwd=0.5) +  
  geom_point(data=filter(example_pairs, label.x1=="p", label.x2=="k"),
             aes(x=xend,y=yend,label=NA), size=2.5) +
  geom_text(data=filter(example_pairs, label.x1=="p", label.x2=="k"),
            aes(x=text_x,y=text_y,
                label=pair_labels(label.x1, label.x2, "/")), size=6) +
  scale_colour_manual(values=emd_palette(example_pairs$Feature)) +
  coord_cartesian(xlim=c(-1.0,1.0), ylim=c(-1.1,1.1)) +
  emd_theme() +
  theme(legend.position="none")
...
names(table_auc_pvals) <- c("Feature", "Acoustic + Articulatory (AUC)",
                            "Acoustic + Articulatory (p)",
                            "Acoustic (AUC)", "Acoustic (p)",
                            "Articulatory (AUC)", "Articulatory (p)")
table_auc_pvals <- select(table_auc_pvals, Feature,
                          `Acoustic (AUC)`,
                          `Acoustic (p)`,
...
All features are represented well in the acoustic + articulatory representation, with the exception of the distinction between the sonorants [j] and [w] and the nearest corresponding obstruents, [ʒ] and [v].

To take a further step towards understanding how well a learner could use these representations to extract linguistically important features from their phonetic experience, we performed a dimensionality reduction to five dimensions using principal component analysis (PCA: CITATION XXX), followed by normalization of the variance to one along these dimensions. For the three representations, the first five PCA dimensions explained `r 100*round(explained_var_pca$pca_variance_proportion[explained_var_pca$encoding_name=="Acoustic"], digits=3)` (Acoustic), `r 100*round(explained_var_pca$pca_variance_proportion[explained_var_pca$encoding_name=="Acoustic + Articulatory"], digits=3)` (Acoustic + Articulatory), and `r 100*round(explained_var_pca$pca_variance_proportion[explained_var_pca$encoding_name=="Articulatory"], digits=3)` (Articulatory) percent of the variance. The orthogonal dimensions derived from PCA are statistically prominent vectors in the data (the top five are the five orthogonal dimensions along which our vector representations vary the most). This representation is in a sense optimal for a learner using no criterion but to find a compact representation of the signal. It still encodes all the same features well.

```{r figure-mptests-pca, warning=F, echo=F}
fig_auc_pvals_pca <- ggplot(mptests_pca_h0_summary, aes(fill=encoding_name, x=fname, y=auc_real)) +
  geom_bar(stat="identity", position="dodge",colour="black") +
  geom_point(aes(y=ifelse(pval < 0.05, 1.00, NA), group=encoding_name),
             shape=21, fill="white",
             position=position_dodge(width=1), size=3) +
  geom_hline(yintercept=0.5) +
  ylim(c(0,1)) +
  scale_fill_manual(values=emd_palette(mptests_pca_h0_summary$encoding_name),
                    name="Encoding") +
  emd_theme() +
  xlab("Feature") +
  ylab("AUC") +
  theme(axis.text.x=element_text(angle=90, hjust=1))
print(fig_auc_pvals_pca)
```

#Conclusions

We have shown that phonetic information that approximates that which would be available in the environment of an infant learning a language contains, in a precise sense, a binary feature structure for English consonants closely corresponding to that posited by linguists as innate structure. This does not necessarily imply that infants do not also have some a priori knowledge of such a structure, but it means that there is no poverty of the stimulus argument to be made about binary distinctive features. The analogy property we tested plays a role in the generalizations formed by learners that govern how words are pronounced: that [z] is the analogical [+voice] correspondent of [s], for example, is an important part of speakers' implicit generalizations about English words. Similar analogical relations are evident across the world's languages. We showed that, at least for English consonants, these analogy properties are part of the signal itself.

One exception was the feature [sonorant]. We believe that it is typologically likely that [sonorant] plays only a classificatory, rather than an analogical, role in the sound patterns of languages, as many languages show phonological alternations with classes defined by this property (for example, voiceless sounds become voiced between sonorants in many languages), but we know of no alternations which change all [-sonorant] sounds into their [+sonorant] correspondent or vice versa. This is a topic for future formal typological investigation.
